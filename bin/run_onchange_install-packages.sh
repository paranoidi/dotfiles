...
# The age identity (private key) is a single shared secret across all
# machines and is never committed to the repo or generated by this script —
# it must be restored from an out-of-band backup on each new machine. This
# just makes a missing key visible instead of a silent decrypt failure later.
check_age_key() {
    local key="${HOME}/.config/chezmoi/key.txt"
    if [[ -f "$key" ]]; then
        echo "✅ age key present"
    else
        echo "⚠️  age key missing at ${key} — restore it from your backup before encrypted chezmoi files will decrypt"
    fi
}
